  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {payslip && (
          <Badge className={
            payslip.status === 'PAID' ? 'bg-green-500' :
            payslip.status === 'VALIDATED' ? 'bg-blue-500' :
            'bg-gray-500'
          }>
            ...
            <div className="bg-muted/30 p-4 rounded-lg">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedMonth}/{selectedYear}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="p-3 bg-blue-50 rounded">
                  ...
                  <p className="text-2xl font-bold text-blue-600">{payslip?.totalHours.toFixed(1) || hoursWorked.toFixed(1)}h</p>
                </div>
                <div className="p-3 bg-orange-50 rounded">
                  <p className="text-sm text-muted-foreground">Heures supplémentaires</p>
                  <p className="text-2xl font-bold text-orange-600">{payslip?.overtimeHours.toFixed(1) || '0'}h</p>
                </div>
                <div className="p-3 bg-purple-50 rounded">
                  <p className="text-sm text-muted-foreground">Heures de nuit</p>
                  <p className="text-2xl font-bold text-purple-600">{payslip?.nightHours.toFixed(1) || '0'}h</p>
                </div>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{payrollCalculator.formatCurrency(payslip.baseSalary)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Prime heures sup:</span>
                      <span className="font-medium">{payrollCalculator.formatCurrency(payslip.overtimePay)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Prime de nuit:</span>
                      <span className="font-medium">{payrollCalculator.formatCurrency(payslip.nightPay)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Prime de risque:</span>
                      <span className="font-medium">{payrollCalculator.formatCurrency(payslip.hazardPay)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Indemnité transport:</span>
                      <span className="font-medium">{payrollCalculator.formatCurrency(payslip.transportAllowance)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Indemnité repas:</span>
                      <span className="font-medium">{payrollCalculator.formatCurrency(payslip.mealAllowance)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm text-red-600">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">-{payrollCalculator.formatCurrency(payslip.socialSecurity)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Impôt sur le revenu (20%):</span>
                      <span className="font-medium">-{payrollCalculator.formatCurrency(payslip.incomeTax)}</span>
                    </div>
                  </div>
                </div>
