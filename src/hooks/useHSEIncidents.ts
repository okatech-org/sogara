...
export function useHSEIncidents() {
  ...
  const createIncident = async (
    ...
    try {
      ...
      try {
        ...
        toast({
          title: 'Incident déclaré (local)',
          description: 'Mode hors-ligne, sauvegardé localement.',
        })
        return { success: true, offline: true } as any
      } catch (e: any) {
        toast({
          title: 'Erreur',
          description: e?.message || "Impossible de créer l'incident.",
          variant: 'destructive',
        })
  ...
  const updateIncident = async (id: string, updates: Partial<HSEIncident>) => {
    try {
      ...
      try {
        ...
        if (updated) {
          setFallbackIncidents(prev =>
            prev ? prev.map(i => (i.id === id ? updated : i)) : [updated],
          )
          toast({
            title: 'Incident mis à jour (local)',
            description: 'Modifications enregistrées hors-ligne.',
          })
          return { success: true, offline: true } as any
        }
        ...
        toast({
          title: 'Erreur',
          description: e?.message || "Impossible de mettre à jour l'incident.",
          variant: 'destructive',
        })
        return { success: false, error: e?.message }
      }
  ...
  const assignInvestigator = async (incidentId: string, investigatorId: string) => {
    try {
      ...
      const updated = await repositories.hseIncidents.update(incidentId, {
        investigatedBy: investigatorId as any,
      })
      if (updated) {
        setFallbackIncidents(prev =>
          prev ? prev.map(i => (i.id === incidentId ? updated : i)) : [updated],
        )
        toast({
          title: 'Enquêteur assigné (local)',
          description: 'Assignation enregistrée hors-ligne.',
        })
        return { success: true, offline: true } as any
      }
      return { success: false }
  ...
  const resolveIncident = async (
    ...
    try {
      ...
      if (updated) {
        ...
        toast({
          title: 'Incident résolu (local)',
          description: 'Résolution enregistrée hors-ligne.',
        })
        return { success: true, offline: true } as any
      }
      return { success: false }
  ...
  const deleteIncident = async (id: string) => {
    try {
      ...
      if (ok) {
        ...
        toast({
          title: 'Incident supprimé (local)',
          description: 'Suppression effectuée hors-ligne.',
        })
        return { success: true, offline: true } as any
      }
      return { success: false }
  ...
  const addIncident = useCallback(
    async (incidentData: Omit<HSEIncident, 'id' | 'createdAt' | 'updatedAt' | 'status'>) =>
