...
export function useEmployeeHSEInbox(employeeId: string) {
  ...
  const myAlerts = useMemo(() => {
    return myAssignments.filter(
      a => a.contentType === 'alert' || a.contentType === 'info' || a.contentType === 'reminder',
    )
  }, [myAssignments])

  const myDocuments = useMemo(() => {
    return myAssignments.filter(a => a.contentType === 'document' || a.contentType === 'procedure')
  }, [myAssignments])

  const unreadCount = useMemo(() => {
    return myAssignments.filter(a => a.status === 'sent' || a.status === 'received').length
  }, [myAssignments])

  const pendingTrainings = useMemo(() => {
    return myTrainings.filter(a => a.status !== 'completed' && a.status !== 'expired')
  }, [myTrainings])
  ...
  const completeTraining = useCallback(
    (assignmentId: string, score: number, certificate?: string) => {
  ...
  const getContentForAssignment = useCallback(
    (assignment: HSEAssignment): HSEContentItem | undefined => {
