...
export function useHSEContent() {
  const [content, setContent] = useState<HSEContentItem[]>(() => getFromStorage(STORAGE_KEY_CONTENT));
  const [assignments, setAssignments] = useState<HSEAssignment[]>(() => getFromStorage(STORAGE_KEY_ASSIGNMENTS));
    ...
    if (content.length === 0) {
      try {
        ...
        const seeded: HSEContentItem[] = modules.slice(0, 6).map((m: any, idx: number) => ({
          ...
          priority: m.category === 'Critique' ? 'critical' : m.category === 'Obligatoire' ? 'high' : 'medium',
          trainingId: m.id,
          createdBy: 'HSE_SEED',
          createdAt: new Date(),
  ...
  const createContent = useCallback((item: Omit<HSEContentItem, 'id' | 'createdAt'>) => {
    try {
  ...
  const assignContent = useCallback((
    ...
    if (!contentItem) {
      ...
      console.log('📋 Content list:', content.map(c => ({ id: c.id, title: c.title })));
      toast({
        title: 'Erreur',
  ...
  const getContentByType = useCallback((type: HSEContentType) => {
    return content.filter(c => c.type === type);
  }, [content]);

  const getSentHistory = useCallback((sentBy: string) => {
    return assignments.filter(a => a.sentBy === sentBy)
      .sort((a, b) => b.assignedAt.getTime() - a.assignedAt.getTime());
  }, [assignments]);

  const getAssignmentsByEmployee = useCallback((employeeId: string) => {
  ...
  const getStats = useMemo(() => ({
    ...
    pendingAssignments: assignments.filter(a => a.status === 'sent' || a.status === 'received').length,
