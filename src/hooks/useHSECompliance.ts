...
export function useHSECompliance() {
  ...
  const checkEmployeeCompliance = useCallback((employeeId: string): EmployeeCompliance | null => {
    ...
    requiredTrainings.forEach(training => {
      ...
      training.sessions.forEach(session => {
        const attendance = session.attendance.find(a => 
          a.employeeId === employeeId && a.status === 'completed'
        );
  ...
  const getServiceCompliance = useCallback((service: string): ComplianceData => {
  ...
  const getRoleCompliance = useCallback((role: UserRole): ComplianceData => {
  ...
  const getComplianceTrends = useCallback((months: number = 6) => {
