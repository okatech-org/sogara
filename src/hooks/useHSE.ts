...
export function useHSE() {
  ...
  const registerForTraining = async (sessionId: string, employeeId: string): Promise<void> => {
    try {
      ...
      toast({
        title: 'Erreur',
        description: "Impossible d'inscrire à la formation",
        variant: 'destructive',
      })
      throw error
    }
    ...
    try {
      ...
      if (filters.startDate && filters.endDate) {
        filteredIncidents = incidents.filter(
          i => i.occurredAt >= filters.startDate && i.occurredAt <= filters.endDate,
        )
      }
      ...
      const reportContent = `Rapport d'incidents HSE\n\nNombre d'incidents: ${filteredIncidents.length}\n\n${filteredIncidents
        .map(i => `${i.type} - ${i.severity} - ${i.occurredAt.toLocaleDateString()}`)
        .join('\n')}`
  ...
  const getComplianceRate = async (): Promise<number> => {
    try {
      ...
      const trainedEmployees = state.employees.filter(
        emp => emp.stats.hseTrainingsCompleted > 0,
      ).length
  ...
  const completeChecklist = async (id: string, responses: any[]): Promise<void> => {
    ...
    toast({
      ...
      description: 'La checklist a été marquée comme complétée',
    })
  }
