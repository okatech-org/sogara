  ...
  const createEmployee = async (employeeData: Omit<Employee, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      ...
      try {
        ...
        toast({
          title: 'Erreur',
          description: e?.message || "Impossible de créer l'employé.",
          variant: 'destructive',
        })
  ...
  const updateEmployee = async (id: string, updates: Partial<Employee>) => {
    try {
      ...
      try {
        ...
        if (updated) {
          setFallbackEmployees(prev =>
            prev ? prev.map(e => (e.id === id ? updated : e)) : [updated],
          )
          toast({
            title: 'Employé mis à jour (local)',
        ...
        toast({
          title: 'Erreur',
          description: e?.message || "Impossible de mettre à jour l'employé.",
          variant: 'destructive',
        })
        return { success: false, error: e?.message }
      }
  ...
  const deleteEmployee = async (id: string) => {
    try {
      ...
      if (ok) {
        ...
        toast({
          title: 'Employé supprimé (local)',
          description: 'Suppression effectuée localement.',
        })
        return { success: true, offline: true } as any
      }
