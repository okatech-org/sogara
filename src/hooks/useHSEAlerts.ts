...
export function useHSEAlerts() {
  ...
  const generateAllHSEAlerts = useCallback(async () => {
    ...
    return newAlerts;
  }, [generateTrainingAlerts, generateComplianceAlerts, generateEquipmentAlerts, state.notifications, dispatch]);
  ...
  const getUnreadHSEAlerts = useCallback(() => {
    const hseTypes = ['hse_training_expiring', 'hse_incident_high', 'hse_equipment_check', 'hse_compliance_alert'];
    return state.notifications.filter(notification => 
      !notification.read && hseTypes.includes(notification.type)
    );
  }, [state.notifications]);

  // Obtenir les alertes HSE par type
  const getHSEAlertsByType = useCallback((type: HSENotification['type']) => {
    return state.notifications.filter(notification => 
  ...
  const getPriorityAlerts = useCallback(() => {
    ...
    const urgentAlerts = state.notifications.filter(notification =>
      priorityTypes.includes(notification.type) && !notification.read
    );

    // Ajouter les formations expirées (jours négatifs)
    const expiredTrainingAlerts = state.notifications.filter(notification =>
    ...
    return [...urgentAlerts, ...expiredTrainingAlerts]
      .sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
  }, [state.notifications]);
