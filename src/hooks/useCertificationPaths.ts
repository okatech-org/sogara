...
export function useCertificationPaths() {
  ...
  const progressData = useQuery(api.certificationPaths.getMyProgress, { candidateId: currentUser?.id || '' });
  ...
  const getMyProgress = useCallback((candidateId: string) => {
    const remote = (progressData || []).map((p: any) => ({
      ...
      evaluationAvailableDate: p.evaluationAvailableDate ? new Date(p.evaluationAvailableDate) : undefined,
      evaluationStartedAt: p.evaluationStartedAt ? new Date(p.evaluationStartedAt) : undefined,
      evaluationSubmittedAt: p.evaluationSubmittedAt ? new Date(p.evaluationSubmittedAt) : undefined,
      evaluationCorrectedAt: p.evaluationCorrectedAt ? new Date(p.evaluationCorrectedAt) : undefined,
      evaluationScore: p.evaluationScore,
      evaluationPassed: p.evaluationPassed,
      habilitationGrantedAt: p.habilitationGrantedAt ? new Date(p.habilitationGrantedAt) : undefined,
      habilitationExpiryDate: p.habilitationExpiryDate ? new Date(p.habilitationExpiryDate) : undefined,
  ...
  const createPath = async (pathData: Omit<CertificationPath, 'id' | 'createdAt' | 'updatedAt' | 'isPublished'>) => {
    try {
