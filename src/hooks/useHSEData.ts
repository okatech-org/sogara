...
export function useHSEData() {
  ...
  const createSampleIncidents = async () => {
    ...
    const sampleIncidents: Omit<HSEIncident, 'id' | 'createdAt' | 'updatedAt' | 'status'>[] = [
      ...
      },
      {
        ...
        severity: 'high',
        description:
          "Petite fuite d'huile détectée près de l'entrée principal. Intervention immédiate des équipes.",
  ...
  const initializeIntelligentTrainingSystem = async () => {
    ...
    if (needsImport()) {
      ...
      console.log(
        `ℹ️ Système de formation déjà initialisé (${stats?.modulesImported} modules, ${stats?.sessionsPlanned} sessions)`,
      )
    }
  }
