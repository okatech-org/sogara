...
export function useHSETrainingImporter() {
  ...
  const importTrainingModules = useCallback(async () => {
    ...
    setImportProgress({
      step: 'importing',
      progress: 10,
      message: 'Importation des modules de formation...',
    })
    ...
    try {
      ...
      setImportProgress({
        step: 'imported',
        progress: 100,
        message: `${importedTrainings.length} modules importés avec succès`,
      })
  ...
  const analyzeTrainingNeeds = useCallback(async (): Promise<TrainingNeed[]> => {
    setImportProgress({
      step: 'analyzing',
      progress: 30,
      message: 'Analyse des besoins de formation...',
    })
    ...
    try {
      ...
      setImportProgress({
        step: 'analyzed',
        progress: 60,
        message: `Analyse terminée pour ${needs.length} employés`,
      })
  ...
  const planTrainingSessions = useCallback(
    async (weeksAhead: number = 12): Promise<HSETrainingSession[]> => {
      setImportProgress({
        step: 'planning',
        progress: 70,
        message: 'Planification des sessions de formation...',
      })
      ...
      try {
        ...
        setImportProgress({
          step: 'planned',
          progress: 90,
          message: `${sessions.length} sessions planifiées`,
        })
  ...
  const generateComplianceReport = useCallback(async (): Promise<ComplianceReport> => {
    setImportProgress({
      step: 'reporting',
      progress: 80,
      message: 'Génération du rapport de conformité...',
    })
    ...
    try {
      ...
      setImportProgress({
        step: 'completed',
        progress: 100,
        message: `Rapport généré - Conformité: ${report.summary.overallCompliance}%`,
      })
  ...
  const simulateTrainingCompletions = useCallback(async () => {
    setImportProgress({
      step: 'simulating',
      progress: 50,
      message: 'Simulation des certifications...',
    })
    ...
    try {
      ...
      setImportProgress({
        step: 'simulated',
        progress: 80,
        message: 'Certifications simulées avec succès',
      })
    } catch (error) {
  ...
  const initializeCompleteHSESystem = useCallback(async () => {
    ...
    try {
      ...
      setImportProgress({
        step: 'planning',
        progress: 50,
        message: 'Planification des sessions...',
      })
  ...
  const getImportStats = useCallback(() => {
    ...
    return {
      ...
      hasRecentImport:
        new Date().getTime() - lastImportResult.importedAt.getTime() < 24 * 60 * 60 * 1000, // Moins de 24h
    }
  }, [lastImportResult])
