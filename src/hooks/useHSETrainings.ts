    ...
    ;(trainings as any[]).forEach((t: any) => {
      ...
      sessions.forEach(s => {
        const att = (s.attendance || []).find(
          (a: HSEAttendance) =>
            a.employeeId === employeeId && a.status === 'completed' && a.expirationDate,
        )
    ...
    ;(trainings as any[]).forEach((t: any) => {
      ...
      sessions.forEach(s => {
        ...
        if (
          ...
          att.expirationDate <= cutoff
        ) {
          const daysUntilExpiry = Math.ceil(
            (att.expirationDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24),
          )
      ...
      if (created) {
        ...
        setFallbackTrainings(prev => {
          ...
          return prev.map((t: any) =>
            t.id === trainingId || t._id === trainingId
