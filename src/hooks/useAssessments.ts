...
export function useAssessments() {
  ...
  const createAssessment = async (assessmentData: Omit<Assessment, 'id' | 'createdAt' | 'updatedAt' | 'isPublished'>) => {
    try {
  ...
  const assignToCandidate = async (assessmentId: string, candidateId: string, candidateType: 'employee' | 'visitor' | 'external') => {
    try {
