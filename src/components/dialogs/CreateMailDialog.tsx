...
export function CreateMailDialog({ trigger }: { trigger?: React.ReactNode }) {
  ...
  const services = useMemo(() => Array.from(new Set(employees.map(e => e.service))).sort(), [employees]);
  ...
  const onSubmit = async () => {
    if (!reference || !sender) {
      toast({ title: 'Champs requis', description: 'Référence et Expéditeur sont obligatoires.', variant: 'destructive' });
      return;
    }
    if (recipientMode === 'employee' && !recipientEmployeeId) {
      toast({ title: 'Destinataire requis', description: 'Sélectionnez un employé destinataire.', variant: 'destructive' });
      return;
    }
    if (recipientMode === 'service' && !recipientService) {
      toast({ title: 'Destinataire requis', description: 'Sélectionnez un service destinataire.', variant: 'destructive' });
      return;
    }
    if (!isConfidential && scannedUrls.length === 0) {
      toast({ title: 'Scan requis', description: 'Ajoutez au moins un scan pour un courrier non confidentiel.', variant: 'destructive' });
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Enregistrer un courrier confidentiel ou non confidentiel.</DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button variant={recipientMode === 'employee' ? 'default' : 'outline'} onClick={() => setRecipientMode('employee')}>Employé</Button>
                <Button variant={recipientMode === 'service' ? 'default' : 'outline'} onClick={() => setRecipientMode('service')}>Service</Button>
              </div>
            ...
            <div className="md:col-span-2">
              ...
              <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Objet / notes..." />
            </div>
            <div className="md:col-span-2 flex items-center gap-2">
              <Checkbox checked={isConfidential} onCheckedChange={(v) => setIsConfidential(Boolean(v))} id="confidential" />
