...
const employeeFormSchema = z.object({
  ...
  matricule: z.string()
    .regex(/^[A-Z]{3}\d{3}$/, 'Le matricule doit suivre le format XXX000 (3 lettres majuscules + 3 chiffres)'),
  
  service: z.string()
    .min(2, 'Le service doit contenir au moins 2 caractères')
    .max(100, 'Le service ne peut pas dépasser 100 caractères'),
  
  roles: z.array(z.enum(['ADMIN', 'HSE', 'SUPERVISEUR', 'RECEP', 'EMPLOYE', 'COMMUNICATION']))
...
export function EmployeeForm({ employee, onSubmit, onCancel, isLoading = false }: EmployeeFormProps) {
