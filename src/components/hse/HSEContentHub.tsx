...
import { Send, BookOpen, AlertTriangle, FileText, Calendar, ArrowRight, Check, Award, Clock } from 'lucide-react';
...
export function HSEContentHub({ employees }: HSEContentHubProps) {
  ...
  const [activeTab, setActiveTab] = useState<'training' | 'alert' | 'document' | 'certification'>('training');
  
  console.log('📊 HSEContentHub - Employés reçus:', employees.length);
  console.log('👤 User connecté:', currentUser?.firstName, currentUser?.lastName, 'ID:', currentUser?.id);
  ...
  const handleSend = () => {
    ...
    switch (activeTab) {
      case 'training':
        if (!selectedTraining) {
          toast({ title: 'Erreur', description: 'Sélectionnez une formation', variant: 'destructive' });
          return;
        }
      ...
      case 'alert':
        if (!alertTitle || !alertMessage) {
          toast({ title: 'Erreur', description: 'Remplissez le titre et le message', variant: 'destructive' });
          return;
        }
        contentItem = createContent({
          type: 'alert',
      ...
      case 'document':
        if (!documentName || !documentUrl) {
          toast({ title: 'Erreur', description: 'Remplissez le nom et l\'URL du document', variant: 'destructive' });
          return;
        }
        contentItem = createContent({
          type: 'document',
  ...
  const handleCandidateSelection = (ids: string[], types: Record<string, 'employee' | 'external'>) => {
  ...
  }, [activeTab, selectedEmployees, selectedCandidates, selectedTraining, alertTitle, alertMessage, documentName, documentUrl, selectedCertificationPath]);
                  ...
                  <SelectContent>
                    {trainingModules.map(module => (
                      <SelectItem key={module.id} value={module.id}>
                        {module.code} - {module.title} ({module.duration}{module.durationUnit})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                ...
                return training ? (
                  <div className="p-4 bg-muted/30 rounded-lg space-y-2">
                    <div className="flex items-center gap-2">
                      <Badge className={
                        training.category === 'Critique' ? 'bg-red-500' :
                        training.category === 'Obligatoire' ? 'bg-orange-500' :
                        'bg-blue-500'
                      }>
                        {training.category}
                      </Badge>
                    ...
                    <div className="flex gap-4 text-xs text-muted-foreground">
                      <span>Durée: {training.duration} {training.durationUnit}</span>
          ...
          <Card className="industrial-card bg-purple-50/30 border-purple-200">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-purple-800">
                Les parcours combinent automatiquement <strong>Formation → Évaluation (après X jours) → Habilitation (si réussite)</strong>
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                <div className="p-3 bg-white rounded border border-purple-200">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Formation assignée immédiatement
                  </p>
                </div>
                ...
                <div className="p-3 bg-white rounded border border-purple-200">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Évaluation débloquée après délai
                  </p>
                </div>
      ...
      {activeTab !== 'certification' && (
        ...
        <div>
          ...
          {!canSend && selectedEmployees.length > 0 && (
            <p className="text-xs text-destructive mt-1">
              ...
              {activeTab === 'alert' && (!alertTitle || !alertMessage) && 'Remplissez le titre et le message'}
              {activeTab === 'document' && (!documentName || !documentUrl) && 'Remplissez le nom et l\'URL'}
            </p>
          )}
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          <Button 
            variant="outline" 
            onClick={() => {
              console.log('👁️ Preview clicked', { canSend, selectedEmployees: selectedEmployees.length });
              if (canSend) {
                setShowPreview(true);
              }
            }}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Badge className="mt-1">
                {activeTab === 'training' ? 'Formation' : 
                 activeTab === 'alert' ? 'Alerte/Information' : 
                 'Document'}
              </Badge>
            </div>

            {activeTab === 'training' && selectedTraining && (() => {
              const training = trainingModules.find(t => t.id === selectedTraining);
              return training ? (
                <div>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              ...
              <Button onClick={() => { handleSend(); setShowPreview(false); }}>
