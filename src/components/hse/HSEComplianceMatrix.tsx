...
export function HSEComplianceMatrix({ employees, trainings }: ComplianceMatrixProps) {
  ...
  const complianceMatrix = useMemo(() => {
    ...
    employees.forEach(employee => {
      ...
      const criticalModules = hseModulesData.hseTrainingModules.filter(
        m => m.category === 'Critique' || m.category === 'Obligatoire',
      )
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-600">{globalStats.fullyCompliant}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-red-600">{globalStats.criticalIssues}</div>
            <p className="text-xs text-muted-foreground">Formations critiques manquantes</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-orange-600">{globalStats.hasExpired}</div>
            <p className="text-xs text-muted-foreground">Employés avec certifications expirées</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="industrial-card">
        <CardHeader>
          ...
          <p className="text-muted-foreground">Vue détaillée de la conformité par employé</p>
        </CardHeader>
        <CardContent>
                  ...
                  {complianceMatrix.slice(0, 20).map(item => (
                    <TableRow key={item.employee.id}>
                      ...
                      <TableCell>
                        <div className="text-xs text-muted-foreground">
                          <div>
                            {item.completed}/{item.totalRequired} complétées
                          </div>
                          {item.missing > 0 && (
                        ...
                        {item.nextExpiring ? (
                          <div className="text-xs">
                            <div
                              ...
                              title={item.nextExpiring.training}
                            >
                              {item.nextExpiring.training.split(' ')[0]}...
                            </div>
                            <div
                              className={`${
                                item.nextExpiring.daysUntil <= 30
                                  ? 'text-orange-600'
          ...
          <CardContent>
            <div className="space-y-4">
              {[...new Set(employees.map(emp => emp.service))].map(service => {
                const serviceEmployees = complianceMatrix.filter(
                  item => item.employee.service === service,
                )

                if (serviceEmployees.length === 0) return null

                const avgCompliance = Math.round(
                  serviceEmployees.reduce((sum, item) => sum + item.rate, 0) /
                    serviceEmployees.length,
                )

                const criticalCount = serviceEmployees.filter(
                  item => item.criticalMissing > 0,
                ).length
            ...
            <div className="space-y-3">
              {Object.entries(
                complianceMatrix.reduce(
                  (acc, item) => {
                    ...
                    requiredTrainings.forEach(training => {
                      const hasCompleted = training.sessions.some(session =>
                        session.attendance.some(
                          att => att.employeeId === item.employee.id && att.status === 'completed',
                        ),
                      )
                ...
                .map(([trainingTitle, count]) => {
                  const module = hseModulesData.hseTrainingModules.find(
                    m => m.title === trainingTitle,
                  )
                  return (
                    <div
                      key={trainingTitle}
