    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      switch (event.key) {
        case 'ArrowDown':
          ...
          setSelectedEmployeeIndex(prev => (prev < eligibleEmployees.length - 1 ? prev + 1 : 0))
          break
        case 'ArrowUp':
          event.preventDefault()
          setSelectedEmployeeIndex(prev => (prev > 0 ? prev - 1 : eligibleEmployees.length - 1))
          break
        case 'Enter':
          event.preventDefault()
            ...
            <div className="flex flex-col gap-3">
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <span className="font-semibold text-blue-900 text-sm sm:text-base line-clamp-1">
                    {module.title}
                  </span>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-blue-700">
                    {module.requiredForRoles.map(getRoleLabel).join(', ')}
                  </span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-3 flex-1 overflow-hidden px-6">
                ...
                <ScrollArea className="h-[280px] sm:h-[350px] pr-2 sm:pr-4">
                  <div className="space-y-2 sm:space-y-3">
                    {eligibleEmployees.map(employee => {
                      const progress = hseTrainingService.getOrCreateProgress(
                        employee.id,
                        module.id,
                      )
                      ...
                      const isSelected =
                        selectedEmployeeIndex === eligibleEmployees.indexOf(employee)
                              ...
                              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                  {isCompleted ? (
                                    <Badge
                                      variant="outline"
                                      className="bg-green-50 text-green-700 border-green-200 gap-1 text-[10px] sm:text-xs"
                                    >
                                    ...
                                    <>
                                      <Badge
                                        variant="outline"
                                        className="bg-blue-50 text-blue-700 border-blue-200 gap-1 text-[10px] sm:text-xs"
                                      >
          ...
          <div className="flex flex-col gap-3 pt-3 border-t bg-muted/30 px-6 py-4">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
              <Button variant="outline" onClick={onCancel} className="gap-2 w-full sm:w-auto">
                <X className="w-4 h-4" />
                Annuler
              </Button>
              ...
              {eligibleEmployees.length > 0 && (
                <div className="text-xs sm:text-sm text-center sm:text-left text-muted-foreground">
                  <span className="font-medium text-green-600">{eligibleEmployees.length}</span>{' '}
                  employé(s) peuvent suivre cette formation
                </div>
              )}
            </div>
