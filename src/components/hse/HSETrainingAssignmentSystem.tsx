...
const DEFAULT_RULES: Omit<AssignmentRule, 'id' | 'createdAt'>[] = [
  {
    ...
    trainingId: 'HSE-001',
    conditions: [{ type: 'role', operator: 'equals', value: 'EMPLOYE' }],
    autoAssign: true,
    priority: 'high',
    reminderDays: 7,
    active: true,
  },
  {
    ...
    trainingId: 'HSE-015',
    conditions: [{ type: 'service', operator: 'equals', value: 'Production' }],
    autoAssign: true,
    priority: 'high',
    reminderDays: 3,
    active: true,
  },
  {
    ...
    trainingId: 'HSE-005',
    conditions: [{ type: 'service', operator: 'equals', value: 'Maintenance' }],
    autoAssign: true,
    priority: 'medium',
    reminderDays: 14,
    active: true,
  ...
  },
  {
    ...
    trainingId: 'HSE-007',
    conditions: [{ type: 'role', operator: 'equals', value: 'SUPERVISEUR' }],
    autoAssign: true,
    priority: 'medium',
    reminderDays: 14,
    active: true,
...
export function HSETrainingAssignmentSystem({
  ...
  const generateAssignments = () => {
    ...
    rules
      .filter(rule => rule.active && rule.autoAssign)
      .forEach(rule => {
  ...
  const getAffectedEmployees = (rule: AssignmentRule): Employee[] => {
    return employees.filter(employee => matchesRuleConditions(employee, rule.conditions))
  }
  ...
  const toggleRuleAutoAssign = (ruleId: string) => {
    setRules(prev =>
      prev.map(rule => (rule.id === ruleId ? { ...rule, autoAssign: !rule.autoAssign } : rule)),
    )
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={generateAssignments} disabled={isRunning} className="gap-2">
              {isRunning ? (
                <>
                ...
                <div className="space-y-4">
                  {rules.map(rule => {
                    ...
                    return (
                      <Card
                        key={rule.id}
                        className={`${rule.active ? 'border-green-200' : 'border-gray-200'}`}
                      >
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                {rule.autoAssign && <Badge variant="secondary">Auto</Badge>}
                              </div>
                ...
                <div className="space-y-6">
                  {[...new Set(employees.map(e => e.service))].map(service => {
                    ...
                    const serviceRules = rules.filter(
                      rule =>
                        rule.active &&
                        rule.conditions.some(c => c.type === 'service' && c.value === service),
                    )
