...
export function HSEMaintenanceTools() {
  ...
  const handleMaintenanceOperation = async (operation: string) => {
    ...
    if (operation === 'clear-all-data' && !confirm('⚠️ ATTENTION : Cette action va supprimer TOUTES les données HSE. Êtes-vous absolument sûr ?')) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert variant="destructive">
        ...
        <AlertDescription>
          <strong>Attention :</strong> Ces outils sont réservés aux administrateurs. 
          Certaines opérations sont irréversibles. Procédez avec prudence.
        </AlertDescription>
      </Alert>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {maintenanceOperations.map((operation) => (
              <Card 
                key={operation.id} 
                className={`border ${
                  operation.severity === 'danger' ? 'border-red-200 bg-red-50' :
                  operation.severity === 'warning' ? 'border-orange-200 bg-orange-50' :
                  'border-border'
                }`}
              >
                <CardContent className="pt-6">
                  <div className="flex items-start gap-4">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-white ${operation.color}`}>
                      {operation.icon}
                    </div>
      ...
      {!canManageMaintenance && (
        <Alert>
          ...
          <AlertDescription>
            Vous n'avez pas les permissions nécessaires pour effectuer des opérations de maintenance.
