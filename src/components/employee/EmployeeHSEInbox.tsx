          ...
          <div className="flex items-start gap-3">
            <div
              className={`p-2 rounded-lg ${
                content.priority === 'critical' || content.priority === 'high'
                  ? 'bg-red-100'
                  : 'bg-yellow-100'
              }`}
            >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                {isUnread && <div className="w-2 h-2 bg-primary rounded-full animate-pulse" />}
              </div>
              <p className="text-sm text-muted-foreground mb-3">
                {content.alertMessage || content.description}
              </p>
              <p className="text-xs text-muted-foreground">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4 className="font-semibold text-foreground mb-1">
                {content.documentName || content.title}
              </h4>
              <p className="text-sm text-muted-foreground mb-2">{content.description}</p>
              <p className="text-xs text-muted-foreground">
        ...
        <div className="space-y-3">
          ...
          <Tabs defaultValue="trainings" className="space-y-3">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="trainings">Formations ({myTrainings.length})</TabsTrigger>
              <TabsTrigger value="alerts">Alertes ({myAlerts.length})</TabsTrigger>
              <TabsTrigger value="documents">Documents ({myDocuments.length})</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="alerts" className="space-y-2">
              {myAlerts.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-6">Aucune alerte</p>
              ) : (
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Formations, alertes et documents personnels</p>
          </div>
        ...
        <Tabs defaultValue="trainings" className="space-y-4">
          ...
          <TabsContent value="trainings" className="space-y-4">
            {myTrainings.length === 0 ? (
              ...
              <div className="space-y-3">{myTrainings.map(renderTrainingCard)}</div>
            )}
          </TabsContent>
          ...
          <TabsContent value="alerts" className="space-y-4">
            {myAlerts.length === 0 ? (
              ...
              <div className="space-y-3">{myAlerts.map(renderAlertCard)}</div>
            )}
          </TabsContent>
          ...
          <TabsContent value="documents" className="space-y-4">
            {myDocuments.length === 0 ? (
              ...
              <div className="space-y-3">{myDocuments.map(renderDocumentCard)}</div>
            )}
          </TabsContent>
        </Tabs>
      </div>
