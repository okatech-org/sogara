...
import { Upload, File, Image, X, CheckCircle, AlertCircle, FileText, Download } from 'lucide-react'
  ...
  const uploadFile = async (uploadedFile: UploadedFile): Promise<void> => {
    return new Promise(resolve => {
      ...
      const interval = setInterval(() => {
        ...
        setUploadedFiles(prev =>
          prev.map(file => (file.id === uploadedFile.id ? { ...file, progress } : file)),
        )
      }, 200)
    })
  }
    ...
    <div className={cn('space-y-4', className)}>
      ...
      {uploadedFiles.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-medium">Fichiers ({uploadedFiles.length})</h4>
            ...
            {uploadedFiles.map(uploadedFile => (
              <Card key={uploadedFile.id} className="p-3">
                ...
                {uploadedFile.status === 'error' && uploadedFile.error && (
                  <div className="mt-2 text-xs text-destructive">{uploadedFile.error}</div>
                )}
