...
export class PackageManagementService {
  ...
  async registerPackageWithAI(
    ...
    const pkg: PackageItem = {
      ...
      signatureRequired: additionalInfo?.signatureRequired || this.requiresSignature(extractionResult.data.packageCategory),
  ...
  async updatePackageStatus(
    ...
    if (status === 'livre') {
      ...
      if (additionalInfo?.signature) this.packages[index].signatureReceived = additionalInfo.signature;
  ...
  async sendReminder(packageId: string): Promise<void> {
    ...
    console.log(`Rappel ${this.packages[index].reminderCount} envoyé pour le colis ${this.packages[index].trackingNumber}`);
  }
  ...
  getPackageStats() {
    ...
    return {
      ...
      recusAujourdhui: this.packages.filter(p => 
