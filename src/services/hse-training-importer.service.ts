...
class HSETrainingImporterService {
  ...
  async importTrainingModules(): Promise<HSETraining[]> {
    ...
    for (const module of hseModulesData.hseTrainingModules) {
      const existingTraining = await repositories.hseTrainings.getAll().then(trainings => 
        trainings.find(t => t.title === module.title)
      );
  ...
  async analyzeTrainingNeeds(): Promise<{
    ...
    for (const employee of employees) {
      ...
      const validTrainings = requiredTrainings.length - missingTrainings.length - expiredTrainings.length;
  ...
  async planTrainingSessions(weeksAhead: number = 12): Promise<HSETrainingSession[]> {
    ...
    const needsByTraining = new Map<string, {
    ...
    for (const [trainingId, data] of needsByTraining) {
      ...
      for (let sessionIndex = 0; sessionIndex < sessionsNeeded; sessionIndex++) {
        ...
        if (isUrgentSession) {
          ...
          sessionDate.setDate(currentDate.getDate() + Math.floor(Math.random() * weeksAhead * 7) + 14); // Après 2 semaines
        }
        ...
        const sessionEmployees = allEmployees.slice(startIndex, startIndex + moduleInfo.maxParticipants);
        ...
        if (createdSession) {
          ...
          console.log(`📅 Session planifiée: ${training.title} - ${sessionDate.toLocaleDateString()} (${sessionEmployees.length} participants)`);
        }
      }
    }
      ...
      for (const session of training.sessions) {
        if (session.status === 'scheduled' && session.date < now) {
          ...
          session.attendance.forEach(attendance => {
            if (Math.random() > 0.1) { // 90% de taux de présence
              ...
              console.log(`🎓 Certification générée pour employé ${attendance.employeeId} - ${training.title}`);
            } else {
  ...
  async generateComplianceReport(): Promise<{
    ...
    const byRole = roles.map(role => {
      ...
      const averageCompliance = roleNeeds.length > 0 
        ? Math.round(roleNeeds.reduce((sum, need) => sum + need.complianceRate, 0) / roleNeeds.length)
        : 100;
      
      const criticalIssues = roleNeeds.reduce((sum, need) => 
        sum + need.expiredTrainings.length, 0
      );
    ...
    const upcomingSessions = trainings.reduce((sum, training) => 
      sum + training.sessions.filter(s => s.status === 'scheduled').length, 0
    );
    ...
    return {
      summary: {
        ...
        overallCompliance,
        criticalTrainings: hseModulesData.hseTrainingModules.filter(m => m.category === 'Critique').length,
        upcomingSessions
      },
      byCategory,
      byRole,
