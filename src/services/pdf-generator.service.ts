...
export class PDFGeneratorService {
  ...
  private static addParticipantInfo(
    pdf: jsPDF,
    employee: Employee,
    margin: number,
    yPosition: number,
  ...
  private static addTrainingInfo(
    pdf: jsPDF,
    module: HSETrainingModule,
    margin: number,
    yPosition: number,
  ...
  private static addCertificationDetails(
    pdf: jsPDF,
    progress: HSETrainingProgress,
    margin: number,
    yPosition: number,
  ...
  private static addSignatures(pdf: jsPDF, margin: number, yPosition: number) {
    ...
    pdf.setFontSize(8)
    pdf.text('Lié Orphée BOURDES', margin + 5, yPosition + 12)
  ...
  static async generateTrainingManualPDF(module: HSETrainingModule): Promise<Blob> {
    ...
    module.content.modules.forEach((contentModule, moduleIndex) => {
      ...
      pdf.setTextColor(100, 100, 100)
      pdf.text(
        `Durée: ${contentModule.duration}h - ${contentModule.description}`,
        margin,
        yPosition,
      )
      yPosition += 8
