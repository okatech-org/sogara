...
class HSETrainingService {
      ...
      if (stored) {
        ...
        return data.map((item: any) => ({
          ...
          certificateIssuedAt: item.certificateIssuedAt
            ? new Date(item.certificateIssuedAt)
            : undefined,
  ...
  getOrCreateProgress(employeeId: string, trainingModuleId: string): HSETrainingProgress {
    ...
    let progress = allProgress.find(
      p => p.employeeId === employeeId && p.trainingModuleId === trainingModuleId,
    )
  ...
  getExpiringTrainings(employeeId: string, withinDays: number = 30): HSETrainingProgress[] {
    ...
    return this.getEmployeeProgress(employeeId).filter(
      p => p.status === 'completed' && p.expiresAt && p.expiresAt <= threshold && p.expiresAt > now,
    )
  }
  ...
  getExpiredTrainings(employeeId: string): HSETrainingProgress[] {
    ...
    return this.getEmployeeProgress(employeeId).filter(
      p => p.status === 'completed' && p.expiresAt && p.expiresAt <= now,
    )
  }
  ...
  generateProgressReport(employeeId: string): {
    ...
    const completed = progress.filter(
      p => p.status === 'completed' && !expired.some(e => e.id === p.id),
    ).length
