...
class ExportService {
    ...
    const columns: ExportColumn[] = [
      {
        header: 'Date',
        key: 'occurredAt',
        formatter: date => format(new Date(date), 'dd/MM/yyyy HH:mm'),
      },
      { header: 'Type', key: 'type' },
      {
        header: 'Gravité',
        key: 'severity',
        formatter: sev => (sev === 'high' ? 'Élevé' : sev === 'medium' ? 'Moyen' : 'Faible'),
      },
      {
        header: 'Employé',
        key: 'employeeId',
  ...
  async generateTrainingReport(
    ...
    const columns: ExportColumn[] = [
      ...
      { header: 'Sessions', key: 'sessions', formatter: sessions => sessions.length.toString() },
      {
        header: 'Créée le',
        key: 'createdAt',
        formatter: date => format(new Date(date), 'dd/MM/yyyy'),
      },
    ]
   ...
   */
  async generateEmployeeReport(employees: Employee[], options: ExportOptions = {}): Promise<Blob> {
    const columns: ExportColumn[] = [
      ...
      { header: 'Email', key: 'email' },
      {
        header: 'Statut',
        key: 'status',
        formatter: status => (status === 'active' ? 'Actif' : 'Inactif'),
      },
...
export const exportVisitsToPDF = (
  visits: Visit[],
  visitors: Visitor[],
  employees: Employee[],
  filename?: string,
...
export const exportEquipmentToPDF = (
  ...
  const columns: ExportColumn[] = [
    ...
    { header: 'Localisation', key: 'location' },
    {
      header: 'Prochain contrôle',
      key: 'nextCheckDate',
      formatter: date => (date ? format(new Date(date), 'dd/MM/yyyy') : 'N/A'),
    },
  ]
...
export const exportPackagesToPDF = (
  ...
  const columns: ExportColumn[] = [
    ...
    },
    {
      header: 'Priorité',
      key: 'priority',
      formatter: priority => (priority === 'urgent' ? 'Urgente' : 'Normale'),
    },
    {
      header: 'Statut',
      key: 'status',
    ...
    },
    {
      header: 'Remis le',
      key: 'deliveredAt',
      formatter: date => (date ? format(new Date(date), 'dd/MM/yyyy HH:mm') : 'Non remis'),
    },
  ]
