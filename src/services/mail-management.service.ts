...
export class MailManagementService {
  ...
  async registerMailWithAI(
    ...
    const mail: Mail = {
      ...
      recipientService:
        extractionResult.data.recipient?.service || additionalInfo?.recipientService,
  ...
  getMailStats() {
    ...
    return {
      ...
      urgents: this.mails.filter(
        m => (m.urgency === 'urgent' || m.urgency === 'tres_urgent') && m.status !== 'archive',
      ).length,
      aTraiter: this.mails.filter(m => m.requiresResponse && !m.responseReceived).length,
      scannesAujourdhui: this.mails.filter(
        m => m.scanDate && new Date(m.scanDate).toDateString() === today,
      ).length,
