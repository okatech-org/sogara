...
export class AIExtractionService {
  ...
  constructor(config?: Partial<AIConfig>) {
    ...
    console.log(
      `🤖 AI Service initialisé - Provider: ${this.config.provider}, Model: ${this.config.model}, API Key présente: ${!!this.config.apiKey}`,
    )
  }
...
Répondez UNIQUEMENT avec le JSON, sans texte supplémentaire.`
  ...
  private async callAIProvider(
    ...
    try {
      ...
      console.log(
        `✅ Extraction réussie en ${processingTime}ms - Confiance: ${Math.round(confidence * 100)}%`,
      )
      ...
      return {
        ...
        requiresVerification:
          confidence < this.config.confidence.verification || warnings.length > 0,
        processingTime,
  ...
  private async callOpenAI(
        ...
        messages: [
          {
            role: 'system',
            content:
              'Tu es un expert en extraction de données depuis des documents. Tu dois répondre UNIQUEMENT avec du JSON valide, sans texte supplémentaire avant ou après.',
          },
          {
            role: 'user',
            content: [
    ...
    try {
      ...
      if (!jsonMatch) {
        throw new Error(
          `Aucun JSON trouvé dans la réponse. Contenu reçu: ${rawContent.substring(0, 200)}`,
        )
      }
  ...
  private async postProcessResult(result: ExtractionResult): Promise<ExtractionResult> {
    ...
    result.requiresVerification =
      result.confidence < this.config.confidence.verification || result.warnings.length > 0
  ...
  private getMockMailResult(): ExtractionResult {
    const mockData = {
      ...
      summary:
        'Communication officielle concernant les nouvelles procédures fiscales à mettre en œuvre à partir du 1er février 2024.',
...
const getDefaultAIConfig = (): AIConfig => {
  ...
  console.warn(
    "💡 Pour activer l'API réelle, ajoutez VITE_OPENAI_API_KEY dans .env.local et redémarrez",
  )
