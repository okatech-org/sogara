...
export class VisitorManagementService {
  ...
  async registerVisitorWithAI(
    ...
    const visitor: VisitorExtended = {
      ...
      idNumber: extractionResult.data.idNumber || extractionResult.data.passportNumber || extractionResult.data.licenseNumber || '',
  ...
  getVisitorStats() {
    ...
    return {
      ...
      todayVisitors: this.visitors.filter(v => 
  ...
  private groupByDepartment(): Record<string, number> {
    return this.visitors.reduce((acc, v) => {
