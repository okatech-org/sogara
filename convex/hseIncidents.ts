...
export const list = query({
  args: {},
  handler: async ctx => {
    const incidents = await ctx.db.query('hseIncidents').order('desc').collect()
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const employee = await ctx.db.get(incident.employeeId)
    const reporter = await ctx.db.get(incident.reportedBy)
    const investigator = incident.investigatedBy ? await ctx.db.get(incident.investigatedBy) : null
...
export const getStats = query({
  ...
  handler: async ctx => {
    ...
    return {
      ...
      highSeverity: allIncidents.filter(i => i.severity === 'high' && i.status !== 'resolved')
        .length,
