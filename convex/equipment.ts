  ...
  handler: async (ctx) => {
    ...
    const enrichedEquipment = await Promise.all(
      equipment.map(async (eq) => {
        const holder = eq.holderEmployeeId
          ? await ctx.db.get(eq.holderEmployeeId)
          : null;
        return {
          ...eq,
  ...
  handler: async (ctx, args) => {
    return await ctx.db
      ...
      .withIndex("by_holder", (q) =>
        q.eq("holderEmployeeId", args.holderEmployeeId)
      )
      .collect();
  },
});
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const holder = equipment.holderEmployeeId
      ? await ctx.db.get(equipment.holderEmployeeId)
      : null;
...
export const getStats = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      needsCheck: allEquipment.filter(
        (eq) => eq.nextCheckDate && eq.nextCheckDate <= nextWeek
      ).length,
      inMaintenance: allEquipment.filter((eq) => eq.status === "maintenance")
        .length,
      operational: allEquipment.filter((eq) => eq.status === "operational")
        .length,
    };
  },
});
